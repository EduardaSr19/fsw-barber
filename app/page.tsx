...
const Home = async () => {
  ...
  const popularBarbsershops = await db.barbershop.findMany({
    orderBy: { name: "desc" },
  })
  ...
  return (
    <div>
      ...
      <div className="p-5">
        ...
        {/* AGENDAMENTO */}
        <h2 className="mb-3 mt-6 font-semibold uppercase text-gray-400">
          Agendamentos
        </h2>
        ...
        <div className="flex gap-4 overflow-auto [&::-webkit-scrollbar]:hidden">
          {popularBarbsershops.map((barbershop) => (
            <BarbershopItem key={barbershop.id} barbershop={barbershop} />
          ))}
        </div>
      </div>
